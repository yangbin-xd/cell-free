...
# predict interference (add distance & direction to edge attr. 
# and only consider interference from several nearest APs)
import os
import torch
import numpy as np
...
class InterfModel(nn.Module):
    def __init__(self, hidden=128, head=4, layers=1):
        ...
        self.edge_mlp1 = nn.Sequential(nn.Linear(2, int(hidden/2)), nn.GELU(),
                                       nn.Linear(int(hidden/2), hidden))
        
        self.edge_mlp2 = nn.Sequential(nn.Linear(2, int(hidden/2)), nn.GELU(),
                                       nn.Linear(int(hidden/2), hidden))
    ...
    def create_edges(self, x, num_aps, num_ues, A, P):
        ...
        edge1_attr = torch.stack([power_s, dist2_s], dim=1).to(device)
        ...
        edge2_attr  = torch.stack([power_i, dist2_i], dim=1).to(device)
...
def test_model(model, test_idx=0):
    ...
    true_value = interf_test[test_idx][:ue_num]
    ...
    print("True interf:", true_value.numpy(), "dB")
    print("Pred interf:", pred_value.numpy(), "dB")
...
if __name__ == "__main__":
    ...
    model = InterfModel()
    model_path = 'model/interf_angle_map.pth'
    ...
    if os.path.exists(model_path):
        ...
        model, train_losses, val_losses = train_model(model, train_loader, val_loader,
                                                      num_epochs=500)
        # Save loss
        np.save('loss/interf_angle_train_losses.npy', train_losses)
        np.save('loss/interf_angle_val_losses.npy', val_losses)
    ...
    np.save('result/pred/pred_interf_angle_mae.npy', mae_flatten)
