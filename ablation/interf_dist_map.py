...
class InterfModel(nn.Module):
    ...
    def create_edges(self, x, num_aps, num_ues, A, P):
        ...
        edge1_attr = torch.stack([power_s, unit_s[:, 0], unit_s[:, 1]], dim=1).to(device)
        ...
        edge2_attr  = torch.stack([power_i, unit_i[:, 0], unit_i[:, 1]], dim=1).to(device)
...
if __name__ == "__main__":
    ...
    model = InterfModel()
    model_path = 'model/interf_dist_map.pth'
    ...
    if os.path.exists(model_path):
        ...
        model, train_losses, val_losses = train_model(model, train_loader, val_loader,
                                                      num_epochs=500)
        # Save loss
        np.save('loss/interf_dist_train_losses.npy', train_losses)
        np.save('loss/interf_dist_val_losses.npy', val_losses)
    ...
    np.save('result/pred/pred_interf_dist_mae.npy', mae_flatten)
