...
class RateModel(nn.Module):
    def __init__(self, signal_model_path='model/signal_map.pth', 
                       interf_model_path='model/interf_intra_map.pth',
...
if __name__ == "__main__":
    ...
    model_path = 'model/rate_intra_map.pth'
    ...
    np.save('result/pred/pred_rate_intra_mae.npy', mae_flatten)
    ...
    if os.path.exists(model_path):
        ...
        model, train_losses, val_losses = train_model(model, train_loader, val_loader, 
                                                      num_epochs=500)
        # Save loss
        np.save('loss/rate_intra_train_losses.npy', train_losses)
        np.save('loss/rate_intra_val_losses.npy', val_losses)
    ...
    mae_flatten = mae_mat.numpy().flatten()
    mae_flatten = mae_flatten[~np.isnan(mae_flatten)]
    np.save('result/pred/pred_rate_intra_finetune.npy', mae_flatten)
    print(f"Test MSE: {mse_loss:.3f}, "
          f"Test RMSE: {np.sqrt(mse_loss):.3f}, "
          f"Test MAE: {mae_loss:.3f}, "
          f"Test SUM: {value:.3f}")
