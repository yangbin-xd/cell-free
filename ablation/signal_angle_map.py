...
# predict signal (add distance to edge attr.)
import os
import torch
import numpy as np
...
class SignalModel(nn.Module):
    def __init__(self, hidden=128, head=4, layers=1):
        ...
        self.edge_mlp = nn.Sequential(nn.Linear(2, int(hidden/2)), nn.GELU(),
    ...
    def create_edges(self, x, num_aps, num_ues, A, P):
        ...
        edge_attr = torch.stack([power, dist2], dim=1).to(device) # [E,2]
...
if __name__ == "__main__":
    ...
    model = SignalModel()
    model_path = 'model/signal_angle_map.pth'
    ...
    if os.path.exists(model_path):
        ...
        model, train_losses, val_losses = train_model(model, train_loader, val_loader,
                                                      num_epochs=1500)
        # save loss
        np.save('loss/signal_angle_train_losses.npy', train_losses)
        np.save('loss/signal_angle_val_losses.npy', val_losses)
    ...
    np.save('result/pred/pred_signal_angle_mae.npy', mae_flatten)
