...
from baseline.radio_map import signal_amp_mean, query_mean_amp
...
for i in np.arange(num_test):
    ...
    for j in np.arange(12):
        amp[:,j], _ = query_mean_amp(ue_loc, UE_train[:,0:2], signal_amp_mean[j])
    ...
    for k in np.arange(ue_num):
        ...
        interf = np.sum(un_served * (amp[k,:])**2)
        noise_dB = -87 - snr
        noise_power = 10 ** (noise_dB / 10)
        #----------------------
        # interf = 10 * np.log10(interf)
        # interf = interf - 3.697
        # interf = 10 ** (interf / 10)
        #----------------------
...
np.save(f'result/query/query_signal.npy', signal_pred)
np.save(f'result/query/query_interf.npy', interf_pred)
np.save(f'result/query/query_rate_{snr}dB.npy', rate_pred)
...
np.save(f'result/query/query_signal_mae.npy', signal_mae)
np.save(f'result/query/query_interf_mae.npy', interf_mae)
np.save(f'result/query/query_rate_mae_{snr}dB.npy', rate_mae)
...
# test case
def baseline(test_idx=0, do_plot=True, do_print=True):
...
if __name__ == '__main__':
    ...
    test_idx = 0
    pred_signal, pred_interf, pred_rate = baseline(test_idx, False, False)